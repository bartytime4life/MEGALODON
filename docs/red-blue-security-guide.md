...
| External alert durable consumer | Confirm caller-backed proxy mutation, unsupported platform, public store mode, duplicate identity, attempt collision, store replacement, schema drift, capacity pressure, stale lock timeout, partial write, lost commit acknowledgement, or false readback cannot claim a clean commit | Linux/non-root/capability-free admission before store access, consumer-owned closed snapshot, descriptor-pinned exact private existing store with no permission repair, no-freelist-credit 512 MiB ceiling, nonwaiting validation probes, remaining-budget lock waits, one `BEGIN IMMEDIATE`, in-transaction replay checks, atomic run/alert/receipt rows, rollback of a provably active failed commit, and exact post-commit readback; only unresolved disposition requires reconciliation | `tests/test_suricata_consumer_preflight.py`, `tests/test_suricata_consumer_runtime.py`, `tests/test_suricata_store.py` |
