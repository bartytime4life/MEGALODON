...
| External alert file reader | Confirm hostile paths, record framing, schema mutations, replay, time/resource pressure, and source changes cannot publish partial or over-authoritative results | Descriptor-retained no-follow open, private-file ownership/mode checks, closed fields, fixed quotas/diagnostics, immutable all-or-nothing publication, no network/process/persistence/action path | `tests/test_suricata_reader.py`, `tests/test_suricata_reader_contract.py` |
