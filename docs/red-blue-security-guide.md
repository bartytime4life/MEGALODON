...
| Dashboard storage | Confirm a missing, unsafe, replaced, linked, or incompatible database cannot be initialized or migrated by the reader | Separate existing-database reader, private POSIX path and inode/entry checks, hardlink and sidecar validation, SQLite `mode=ro`, `query_only`, exact-schema validation, and a five-field projection; independent review and native Windows ACL evidence remain unproved. [#66](https://github.com/bartytime4life/MEGALODON/issues/66) closed 2026-09-12 as a design/fixture delivery; it does not assert that evidence exists. Native Windows ACL proof was deferred to [#27](https://github.com/bartytime4life/MEGALODON/issues/27), also closed (2026-09-14) with its own notice that native execution "remains a separate handoff" and no issue currently tracks it | `tests/test_storage.py`, `tests/test_dashboard_binding.py`, `tests/test_dashboard.py` |
...
| Reference Library context | Try to turn a port/protocol registration into observed-service classification, threat reputation, or a hidden-evidence join; tamper with a shard or replay malformed query fields | Full manifest/shard/count/hash/schema verification before lookup, GET-only normalized keys, finite cache/results, provenance and warning on every result, one path-free integrity failure, and no store/action/update/egress path | `tests/test_reference_data.py`, `tests/test_dashboard.py` |
...
| External alert durable consumer | Confirm caller-backed proxy mutation, unsupported platform, public store mode, duplicate identity, attempt collision, store replacement, schema drift, capacity pressure, stale lock timeout, partial write, lost commit acknowledgement, false readback, or ambiguous reconciliation cannot claim a clean commit or absence | Linux/non-root/capability-free admission before store access, consumer-owned closed snapshot, descriptor-pinned exact private existing store with no permission repair, no-freelist-credit 512 MiB ceiling, remaining-budget lock waits, one atomic write transaction, exact post-commit readback, and a separate query-only snapshot that proves the complete run/alert/attempt/receipt tuple or exact dual absence; every mismatch and storage failure is indeterminate | `tests/test_suricata_consumer_preflight.py`, `tests/test_suricata_consumer_runtime.py`, `tests/test_suricata_reconciliation.py`, `tests/test_suricata_store.py` |
| Resource pressure | Confirm configured counts, windows, state, files, subprocess output, API results, and reports fail closed at their limits | Existing finite validation and per-component caps; whole-service storage and overload acceptance remains unproved. [#68](https://github.com/bartytime4life/MEGALODON/issues/68) closed 2026-09-12 (its dependency-ordered slices landed); [SECURITY_REVIEW.md](../SECURITY_REVIEW.md#open-control-register)'s whole-service resource-bounds row still lists portable/Windows interruption, installed-capture loss and long-running exhaustion evidence as outstanding regardless of that closure | `tests/test_detector.py`, `tests/test_detector_acceptance.py`, `tests/test_config.py`, `tests/test_offline.py` |
...
or response. The durable consumer authorizes only the exact local transaction
described above, and its reconciler authorizes only query-only evidence
classification; neither grants watcher, sensor, UI, or response authority. AI
integration is not an earlier shortcut.
