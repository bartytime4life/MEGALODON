...
The three-path capture lifecycle change entered `main` at
`fb2563ba97ee05526c60c45cadbd68f790a5236c` through merged PR #106. That is
the delivery commit for this narrow slice, not a floating claim about the
repository's current `main`. The recorded candidate workflow run `34664505722`
reported `1058` passed tests with `1` skip, including wheel-smoke; its result
belongs to that exact tested tree. The merge and hosted result are delivery
evidence, not independent approval under issue #3. The change adds observability
and fail-closed handling around the optional Scapy adapter; it does not activate
