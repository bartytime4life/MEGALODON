...
| --- | --- | --- |
| Trust-kernel work must first implement firewall containment, separate dashboard reads, and atomic events | **STALE as implementation backlog.** These controls exist; their acceptance limits remain separate. | `megalodon/firewall.py`, `dashboard_integrations.py`, `storage.py`, `tests/test_storage_failures.py`, and the open-control register in `SECURITY_REVIEW.md`. Do not recreate them or claim every operating environment is accepted. |
