...
- On supported Linux runtimes, optional `--max-seconds N` values from 1 through 86,400
  ...
  teardown cancels it. Interrupted competing-timer restoration is read back: a
  confirmed restored timer is preserved, while an incomplete swap cancels the
  deadline and restores the displaced timer before handler restoration. An already active
  ...
  entry is re-raised only after teardown completes. Dispatch as setup unmasking
  returns causes teardown to reblock before cancellation; dispatch during
  post-cancel timer inspection is preserved until remaining cleanup decisions
  complete. Cancellation restores the prior handler
