...
| Suricata EVE integration | Single-threaded Linux main-thread reader for one completed private contract-envelope file, an explicit atomic transaction into one pre-created private store, and explicit read-only unknown-commit reconciliation; no raw-EVE converter, watcher, dashboard, sensor, or IPS | Contract only; native Suricata availability does not change this |
