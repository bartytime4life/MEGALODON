...
## Generated Wiki ownership

The Wiki publisher projects `docs/wiki/*.md`, `_Sidebar.md`, and `_Footer.md`
into a temporary Wiki checkout. `tools/sync_wiki_pages.py` records generated
page names and SHA-256 digests in `.megalodon-wiki-pages.json` inside that Wiki.
On later publications, a removed or renamed source page is deleted only when
its old generated bytes still match the recorded digest. An edited obsolete
page stops publication before any page changes; reconcile that page explicitly
before retrying. Unrelated manually maintained pages remain untouched.

The first publication with this helper adopts only the current generated page
names. Older orphan pages have no ownership record and remain for explicit
maintainer cleanup; the helper does not infer ownership from filenames or page
contents. Current generated names remain source-controlled and are replaced by
`docs/wiki` content, including on first adoption. Edit those pages in the source
repository rather than through the Wiki editor.

The helper preflights names, ownership and affected paths, refuses symlinks and
nonregular inputs, then writes the ownership manifest last. It expects exclusive
use of trusted local directories. Filesystem failure can leave that disposable
checkout partially updated; the shell stops without committing or pushing.
It provides no network, Git or publication operation itself. The shell validation
mode exercises the same helper against an empty disposable checkout, so incoming
page-policy failures are caught before publication. Verify locally with:

```bash
python -m pytest -q tests/test_wiki_sync.py
bash .github/scripts/sync-wiki.sh --validate
```

The offline tests cover adoption, pruning, preservation and refusal, plus the
publish shell path against a temporary local bare Git repository. They do not
contact GitHub or certify a hosted publication.
