...
_WORKFLOWS = (
    ...
    },
    {
        ...
        "integration_owner": "megalodon.offline.suricata_eve + megalodon.offline.suricata + megalodon.offline.suricata_consumer",
        "input_contract": "pinned Suricata 8.0.7 alert-only EVE file or suricata-eve-alert-input-v1 envelope",
        "output_contract": "immutable external-alert-v1 publication plus terminal consumer or reconciliation receipt",
        "entry_point": "Python APIs: read_completed_raw_eve, read_completed_file, consume_publication, reconcile_publication",
        "launch_policy": "explicit_checksum_bound_completed_file_conversion_or_envelope_read_then_explicit_local_transaction_or_read_only_reconciliation",
        "data_boundary": "closed alert fields from the pinned privacy-minimal EVE profile or envelope; no payload, packet, flow, application object, file content, unrestricted protocol record, or ruleset update",
        ...
        "next_gate": "installed Suricata 8.0.7 producer and operational loss/privacy acceptance; startup snapshots require restart to refresh",
    },
    {
    ...
    },
    {
        ...
        "integration_owner": "megalodon.qwen_advisory",
        "input_contract": "local-model-advisory-v1 request after fingerprint-pinned Airlock preflight",
        "output_contract": "bounded advisoryResult with model provenance and fixed limitations",
        "entry_point": "Python API: invoke_qwen_advisory",
