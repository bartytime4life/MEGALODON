...
def validate_packet_metadata(value: Any) -> dict[str, str]:
    ...
    the core.  This extension point is therefore deliberately narrow: it may
    attest which reviewed adapter produced a record, but it may not become a
    generic channel for packet content, hashes, credentials, or arbitrary
    sensor data.
    """
    if value is None:
        return {}
