...
class DashboardReader(Protocol):
    """The narrow storage interface available to the HTTP presentation layer."""
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if route.path == "/api/summary":
            try:
                ...
                self._send_json({"error": "dashboard data unavailable"}, status=503)
                return
...
def _dashboard_events(
    store: DashboardReader, limit: int
) -> list[dict[str, Any]]:
