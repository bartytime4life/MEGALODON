...
support from an installed binary: MEGALODON has one pinned, checksum-bound 8.0.7
alert-only EVE converter, a completed contract-envelope reader, an explicit
...
| [#256 — SQLite recovery contract](https://github.com/bartytime4life/MEGALODON/issues/256) | Contract, standalone engine, explicit operator workflow, and focused Linux lock/SIGINT/path-race/file-limit tests are present; backup/restore remain online-backup-only, new-destination, fixed-bound, and receipted | Native power-loss/physical-disk-full/high-write-WAL/Windows evidence, operator acceptance, independent review, configuration activation, release, or deployment |
