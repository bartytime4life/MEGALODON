...
| Local Qwen advisory boundary | [`megalodon/qwen_advisory.py`](megalodon/qwen_advisory.py), [`megalodon/provider_containment.py`](megalodon/provider_containment.py), [`docs/local-model-advisory-contract.md`](docs/local-model-advisory-contract.md), [`docs/model-containment-review.md`](docs/model-containment-review.md), [`docs/qwen-provider-hardening.md`](docs/qwen-provider-hardening.md), and [`contracts/local-model-advisory/v1`](contracts/local-model-advisory/v1/README.md) |
...
| SQLite backup/restore contract and its bounded engine | [`docs/sqlite-recovery-contract.md`](docs/sqlite-recovery-contract.md), [`contracts/sqlite-recovery/v1`](contracts/sqlite-recovery/v1/README.md), and [`megalodon/sqlite_recovery.py`](megalodon/sqlite_recovery.py) |
...
| SQLite backup/restore engine | Contract-defined bounded backup-to-new-artifact and restore-to-new-destination (`megalodon/sqlite_recovery.py`) using SQLite's online-backup API only, descriptor-safe admission, full integrity/foreign-key verification, and one closed terminal receipt; no CLI/API wiring, retention, or native failure-injection evidence yet |
...
as a standalone, callable engine (`backup_database`, `restore_backup`); it is
not wired into any CLI subcommand, API route, dashboard control, or scheduled
job. There is no general backup or restore command, no in-place restore or
overwrite, and no automatic cleanup, retention, migration, repair,
configuration change, or dashboard write path.
...
| [#256 — SQLite recovery contract](https://github.com/bartytime4life/MEGALODON/issues/256) | Contract delivered by merged PR #271: explicit backup/restore requests, online-backup-only policy, new-destination identity, fixed bounds, closed receipts/reasons, adversarial fixtures, and a fault runbook. A standalone bounded engine (`megalodon/sqlite_recovery.py`) now implements it, validated in `tests/test_sqlite_recovery_engine.py`; issue remains open at the 2026-09-17 readback | Native failure injection (crash, disk-full, power-loss), operator acceptance, independent review, CLI/API wiring decision, configuration activation, release, or deployment |
