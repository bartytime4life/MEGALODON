...
@pytest.mark.parametrize('field,value', [
    ...
    ('file_uri', 'file:///SECRET'), ('role', 'system'),
    ('approval', 'operator-approved'), ('recovery', {'endpoint': 'http://198.51.100.1'}),
])
def test_closed_request_denies_injection_and_spoofing(field, value, monkeypatch):
    ...
    # Integration with the enabled provider must not turn a rejected field
    # into a request, an indirect resource read, or a claimed recovery action.
    ...
    enabled_result = invoke(request=request)
    assert enabled_result.decision == 'DENY'
    assert enabled_result.reason_code == result.reason_code
    assert enabled_result.provider_request_performed is False
    assert 'SECRET' not in json.dumps(enabled_result.to_dict())
