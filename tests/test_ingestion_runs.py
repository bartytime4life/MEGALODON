...
def test_run_finalization_does_not_scan_historical_detections(tmp_path):
    with Store(tmp_path / "audit.db") as store:
        run_id = store.start_ingestion_run("sample", started_at=STAMP)
        event_id = store.record_event(_event(), run_id=run_id)
        store.record_detection(
            event_id,
            DetectionResult(
                STAMP,
        ...
        def refuse_detection_reads(action, arg1, *_args):
            if action == sqlite3.SQLITE_READ and arg1 == "detections":
                return sqlite3.SQLITE_DENY
            return sqlite3.SQLITE_OK

        store.connection.set_authorizer(refuse_detection_reads)
        try:
            receipt = store.finish_ingestion_run(
                run_id, "completed", finished_at=STAMP + timedelta(seconds=1)
            )
...
def test_detection_and_run_counter_roll_back_together(tmp_path):
    with Store(tmp_path / "audit.db") as store:
        run_id = store.start_ingestion_run("sample", started_at=STAMP)
        event_id = store.record_event(_event(), run_id=run_id)
        store.connection.execute(
        ...
        assert store.connection.execute(
            "SELECT detection_count FROM ingestion_runs WHERE id = ?", (run_id,)
        ).fetchone()[0] == 0
