...
async def exercise(browser, port: int, nonempty: bool) -> None:
    ...
    try:
        ...
        await expect(page.locator("#workspace-live")).to_be_hidden()
        await expect(page.locator("#workspace-analysis")).to_be_visible()
        await page.locator('#ingestion-runs-panel > summary').click()
        await expect(page.locator('#ingestion-runs-status')).to_contain_text('1 newest receipt')
        await page.locator('#ingestion-source').select_option('sample')
        await expect(page.locator('#ingestion-runs-status')).to_have_text('No recorded sample runs')
        await page.locator('#ingestion-source').select_option('jsonl')
        await expect(page.locator('#ingestion-runs-status')).to_contain_text('1 newest jsonl receipt')
        await expect(page.locator('#ingestion-runs-list')).to_contain_text('jsonl')
        await page.locator('#ingestion-source').select_option('all')
        await expect(page.locator('#ingestion-runs-status')).to_contain_text('1 newest receipt')
        passed('real browser source filter shows qualified stored receipts')
