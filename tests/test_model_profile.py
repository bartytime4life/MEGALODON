...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda value: value.__setitem__("thinking_enabled", True), "THINKING_MUST_BE_DISABLED"),
        (lambda value: value.__setitem__("structured_output", False), "STRUCTURED_OUTPUT_REQUIRED"),
        (lambda value: value.__setitem__("operational_context", 4097), "OPERATIONAL_CONTEXT"),
        (lambda value: value.__setitem__("extra", "field"), "PROFILE_SHAPE"),
        (lambda value: value["runner"].__setitem__("extra", "field"), "RUNNER_SHAPE"),
    ],
)
...
def test_profile_digest_is_order_independent_and_mutation_sensitive():
    ...
    assert validate_profile(first).canonical_sha256 == validate_profile(second).canonical_sha256
    second["evaluation"] = deepcopy(second["evaluation"])
    second["evaluation"]["p95_latency_ms"] = 901
    assert validate_profile(first).canonical_sha256 != validate_profile(second).canonical_sha256


def test_comparison_is_deterministic_and_does_not_select():
    first = profile()
    first["profile_id"] = "qwen25-a"
    second = profile()
    second["profile_id"] = "qwen25-b"
    second["evaluation"]["p95_latency_ms"] = 800
    comparison = compare_profiles([validate_profile(first), validate_profile(second)])
    assert [item["profile_id"] for item in comparison["ranking"]] == ["qwen25-b", "qwen25-a"]
...
def test_canonical_json_contains_no_incidental_whitespace():
    rendered = canonical_json(binding_candidate_packet(validate_profile(profile())))
