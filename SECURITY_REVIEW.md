...
| Medium | GUI threat map can create a false precision problem | IP geolocation can expose or misrepresent people and locations | The local HUD's optional browser-only activity globe accepts a bounded operator-selected exact-IP CSV, rounds coordinates to about five degrees, and labels locations as approximate context. It has no external lookup, saved mapping, origin claim, or response authority. |
