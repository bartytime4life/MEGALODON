...
| SQLite recovery contract ([#256](https://github.com/bartytime4life/MEGALODON/issues/256)) | Contract candidate closes explicit backup/restore requests, online-backup-only and new-destination invariants, fixed capacity/deadline limits, path-free terminal receipts, reason codes, adversarial fixtures, and the required fault matrix | Implement nothing until separately reviewed; then prove descriptor safety, WAL/lock behavior, physical exhaustion, interruption, corruption, clock rollback, uncertain completion, and operator recovery on an authorized host. No automatic cleanup, activation, release, or deployment follows |
...
implemented as a bounded source candidate with exact digest, private-file,
