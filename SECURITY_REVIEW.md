...
| SQLite recovery contract ([#256](https://github.com/bartytime4life/MEGALODON/issues/256)) | Contract delivered by merged PR #271 closes explicit backup/restore requests, online-backup-only and new-destination invariants, fixed capacity/deadline limits, path-free terminal receipts, reason codes, adversarial fixtures, and the required fault matrix; issue remains open at the 2026-09-17 readback | Runtime implementation requires separate review; then prove descriptor safety, WAL/lock behavior, physical exhaustion, interruption, corruption, clock rollback, uncertain completion, and operator recovery on an authorized host. No automatic cleanup, activation, release, or deployment follows |
...
The [external exchange contract](docs/external-exchange-contract.md) keeps all
three lanes local and non-authoritative. The completed-file STIX reader is now
implemented on `main` through merged PR #269 with exact digest, private-file,
closed projection, marking-resolution, immutable-output, and no-external-work
controls. It does not authenticate feed content, fetch TAXII, persist context,
evaluate patterns, invoke a model, or grant detection, attribution, or action
authority. Bounded local ECS/OCSF output remains contract-only, and the SOAR
lane still has no destination, endpoint, credential, retry, automation, or
action. Live TAXII, SIEM delivery, webhooks, provider credentials, background
jobs, and SOAR mutation remain unimplemented and unauthorized.
