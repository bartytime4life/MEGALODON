...
| Medium | A local language-model integration can leak telemetry, follow redirects or proxies, hallucinate authority, or exhaust resources | Sensitive disclosure, accidental egress, misleading verdicts, and an action-confused UI | The adapter reruns the pinned metadata-only Airlock, connects only to literal `127.0.0.1:11434`, exposes no endpoint or raw-prompt option, follows no redirects, reads no proxy or DNS configuration, and bounds concurrency, deadline, cancellation, request, HTTP framing, response body, generation and display text. The original run-count policy is library-only; the separate offline anomaly command requires explicit enablement and preserves evidence independently. Neither policy grants dashboard invocation, storage, detector or response authority |
