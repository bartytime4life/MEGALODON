...
| --- | --- | --- |
| Aggregate JSONL input budget ([#203](https://github.com/bartytime4life/MEGALODON/issues/203)) | Every physical JSONL line counts toward a fixed 256 MiB UTF-8 budget before classification or parsing; the first excess line fails with a record-free capture error and the suffix remains unread | Revalidate exact-head behavior on intended Linux files and pipes; retain the separate event, skipped-line, elapsed-deadline, storage, and cleanup controls |
