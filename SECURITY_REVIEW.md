...
| Medium | A local language-model integration can leak telemetry, follow redirects/proxies, hallucinate authority, or exhaust resources | Sensitive disclosure, accidental egress, misleading verdicts, and an action-confused UI | The optional internal adapter reruns the pinned metadata-only Airlock, connects only to literal `127.0.0.1:11434`, exposes no endpoint/raw-prompt/tool option, follows no redirects, reads no proxy or DNS configuration, and bounds concurrency, end-to-end time, cancellation, request, response, tokens, and display text. Its receipt is untrusted advisory text and is not wired to CLI, dashboard, storage, detection, or response |
...
| Whole-service resource bounds ([#68](https://github.com/bartytime4life/MEGALODON/issues/68)) | Detector windows, capture queues, API result sets, offline inputs, subprocess output, storage intake, and internal retention transactions have finite limits; each retention apply is bound to an exact preview and deletes at most 256 explicit rows | Select operator retention values and prove installed-capture loss behavior, native long-running storage/exhaustion, and any future delivery queue before claiming continuous monitoring |
...
| Local Qwen advisory ([#165](https://github.com/bartytime4life/MEGALODON/issues/165)) | Fingerprint-pinned Airlock plus an internal literal-loopback, non-streaming, concurrency-one provider boundary with finite non-echoing receipts and adversarial fake-provider tests | Verify a real operator-owned model alias/artifact and installed Ollama lifecycle on an authorized host; complete independent review before user-facing exposure; keep dashboard, persistence, background operation, and action authority absent |
